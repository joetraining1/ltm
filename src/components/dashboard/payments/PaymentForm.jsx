...
const PaymentForm = ({ onClose, title, nama, desc, id, refresh, port }) => {
  ...
  const upApi = async () => {
    ...
    try {
      const reqData = await ApiClient.put(`payment/${id}`, payload).then(
        (response) => {
  ...
  const newApi = async () => {
    ...
    try {
      const reqData = await ApiClient.post("payment", payload).then(
        (response) => {
          return response.data;
        }
      );
  ...
  return (
    <React.Fragment>
      ...
      >
        <AtmOutlinedIcon sx={{ color: colorHex.iconColor }} />
        <Divider
